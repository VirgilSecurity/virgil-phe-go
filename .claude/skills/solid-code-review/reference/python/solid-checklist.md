...
- Circular imports between modules (signals tangled responsibilities — causes `ImportError`/`AttributeError` at runtime)
- **Ask**: "What is the single reason this module would change?"

## OCP (Open/Closed)

- Adding a new behavior requires editing many `if/elif` chains (or `match/case` blocks)
...
- Subclass checks for concrete type via `isinstance` or raises for base method
...
- `NotImplementedError` stubs in subclasses
...
| **Expensive `@property`** | Property getter performs I/O, DB queries, or heavy computation — callers expect attribute access to be cheap |
...
6. **Prefer composition over inheritance** - Multiple inheritance creates MRO complexity and tight coupling
