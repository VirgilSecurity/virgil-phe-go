...
- Large class with low cohesion or multiple reasons to change
- Functions that orchestrate many unrelated steps
- God objects that know too much about the system
- **Ask**: "What is the single reason this class would change?"

## OCP (Open/Closed)

- Adding a new behavior requires editing many `when`/`if` blocks
- Feature growth requires modifying core logic rather than extending
- No plugin/strategy/hook points for variation; sealed classes not leveraged for exhaustive handling
- **Ask**: "Can I add a new variant without touching existing code?"

## LSP (Liskov Substitution)

- Subclass checks for concrete type via `is` or throws `UnsupportedOperationException`
...
- Empty/stub/`TODO()` implementations of interface methods
...
| **Primitive obsession** | Using strings/numbers instead of domain types (consider `value class`) |
...
1. **Split by responsibility, not by size** - A small class can still violate SRP
...
7. **Make illegal states unrepresentable** - Use sealed classes and types to enforce invariants
