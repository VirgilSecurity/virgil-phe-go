...
- Methods that orchestrate many unrelated steps
...
- No plugin/strategy/hook points for variation; sealed classes (Java 17+) not leveraged for exhaustive handling
- **Ask**: "Can I add a new variant without touching existing code?"

## LSP (Liskov Substitution)

- Subclass checks for concrete type via `instanceof` or throws `UnsupportedOperationException`
...
- Empty/stub/`UnsupportedOperationException` implementations of interface methods
...
| Smell | Signs |
|-------|-------|
| **Long method** | Method > 30 lines, multiple levels of nesting |
